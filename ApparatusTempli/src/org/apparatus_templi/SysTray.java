...
public class SysTray implements ActionListener {
	...
	private Image imgWaiting = null;
	// = Toolkit.getDefaultToolkit().getImage(
	// getClass().getResource("/icons/waiting32x32.png"));
	private Image imgRunning = null;
	// = Toolkit.getDefaultToolkit().getImage(
	// getClass().getResource("/icons/icon32x32.png"));
	private Image imgTerm = null;
	// = Toolkit.getDefaultToolkit().getImage(
	// getClass().getResource("/icons/term32x32.png"));
	private boolean iconExists = false;

	public SysTray() {
		try {
			try {
				imgWaiting = Toolkit.getDefaultToolkit().getImage(
						getClass().getResource("/icons/waiting32x32.png"));
				imgRunning = Toolkit.getDefaultToolkit().getImage(
						getClass().getResource("/icons/icon32x32.png"));
				imgTerm = Toolkit.getDefaultToolkit().getImage(
						getClass().getResource("/icons/term32x32.png"));
			} catch (NullPointerException e) {
				Log.t(TAG, "unable to load icon resource: " + e.getMessage());
				Coordinator.exitWithReason("unable to load systray icon resource");
			}
	...
	private class AboutDialog extends JDialog {
		...
		public AboutDialog() {
			...
			largeIconLabel.setIcon(new ImageIcon(getClass().getResource("/icons/large_logo.png")));
			largeIconLabel.setBounds(6, 6, 332, 89);
			contentPanel.add(largeIconLabel);

			JLabel versionNumLabel = new JLabel("v " + Coordinator.getVERSION_NUMBER_MAJOR() + "."
					+ Coordinator.getVERSION_NUMBER_MINOR());
