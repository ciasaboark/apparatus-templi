...
public class Coordinator {
	...
	// version information
	private static String VERSION_NUMBER_MAJOR;
	private static String VERSION_NUMBER_MINOR;
	...
	public static String getVERSION_NUMBER_MAJOR() {
		return VERSION_NUMBER_MAJOR;
	}

	public static String getVERSION_NUMBER_MINOR() {
		return VERSION_NUMBER_MINOR;
	}

	public static void main(String argv[]) {
		// read in version information from properties file
		Properties versioning = new Properties();
		try {
			versioning.load(Coordinator.class.getResourceAsStream("/versioning"));
			VERSION_NUMBER_MAJOR = versioning.getProperty("version.major");
			VERSION_NUMBER_MINOR = versioning.getProperty("version.minor");
			// abuse the IOException catch below
			if (VERSION_NUMBER_MAJOR == null || VERSION_NUMBER_MINOR == null) {
				throw new IOException("read one of major or minor version number as null");
			}
			for (Object key : versioning.keySet()) {
				System.out.println("key: " + versioning.get(key));
			}
		} catch (IOException e) {
			Log.t(TAG, "error reading version information: " + e.getMessage());
			Coordinator.exitWithReason("error reading version information");
		}
		...
		Runtime.getRuntime().addShutdownHook(new Thread() {
			@Override
			public void run() {
				Log.w(TAG, "system is going down. Notifying all drivers.");
