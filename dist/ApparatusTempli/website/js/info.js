	...
	if (document.getElementById('sys_status') !== null) {
		...
		$.ajax({
            ...
            success: function(text) {
				var versionText;
				if (text === "") {
					versionText = "<p>Version: unknown</p>";
				} else {
					versionText = "<p>Version: " + text + "</p>";
				}
				console.log(text);
				document.getElementById('sys_status_version').innerHTML = versionText;
			},
			error: function(xhr, status, error) {
				console.log("unable to read version number: " + error);
				var text = "<p>Version: unknown</p>";
				console.log(text);
				document.getElementById('sys_status_version').innerHTML = text;
			}	
		});
