...
public final class EmailService implements ServiceInterface {
	...
	/**
	 * Send a new email message on the user. The user must be specified with the public/private key
	 * pair in {@link Prefs.Keys#twtrAccess} and {@link Prefs.Keys#twtrAccessKey} . If no user id or
	 * password has been configured no email message will be sent.
	 * 
	 * @param subject
	...
	public synchronized boolean sendEmailMessage(String recipients, String subject, String message) {
		...
		if (message != null && !message.equals("") && subject != null && !subject.equals("")
				&& recipients != null && !recipients.equals("")) {
			try {
			...
			Log.w(TAG,
					"Can not send email message, check that the user id and password was provided");
		}
		return messageSent;
	}
