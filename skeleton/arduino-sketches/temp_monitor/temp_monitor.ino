...
const String MODULE_NAME = "TempMonitr";
...
void executeCommand(String command) {
	...
	if (command.compareTo("t") == 0) {
		//read temperature
                flashLED(5, 2);
                float t = dht.convertCtoF(dht.readTemperature());
                //convert to the closest int value
                int f = t >= 0 ? (long)(t+0.5) : (long)(t-0.5);
                String temp = "t" + String(f);
                sendCommand(temp);
                ...
                //convert to the closest int value
                int f = h >= 0 ? (long)(h+0.5) : (long)(h-0.5);
                String humid  = "h" + String(f);
                sendCommand(humid);
